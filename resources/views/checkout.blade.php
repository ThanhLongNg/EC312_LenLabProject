...
<script>
    ...
    // Lưu draft thông tin khách hàng
    let draftTimer = null;
  ...
  function saveDraft() {
    ...
    $.ajax({
      url: '/checkout/save-draft',
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      contentType: 'application/json',
      data: JSON.stringify({ full_name, phone, email }),
      success: function(res) {
        // console.log('draft saved', res);
      },
      error: function(xhr) {
        // console.log('draft error', xhr.responseText);
      }
    });
  }

  // lưu khi user rời khỏi input (blur) + debounce 300ms
  $(document).on('blur', 'input[name="email"], input[name="phone"], input[name="full_name"]', function () {
    clearTimeout(draftTimer);
    draftTimer = setTimeout(saveDraft, 300);
  });

  // (tuỳ chọn) lưu khi user thoát trang

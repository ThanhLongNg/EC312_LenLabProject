    ...
    public function saveDraft(Request $request)
{
    ...
    if (!$request->email && !$request->phone) {
        return response()->json(['success' => false, 'message' => 'Thiếu email hoặc phone'], 422);
    }
    ...
    return DB::transaction(function () use ($request, $userId, $cartItems) {
        ...
        if (!$order) {
            $order = Order::create([
                'order_id' => 'DRAFT-' . strtoupper(Str::random(10)),
                'user_id'  => $userId,
                'status'   => 'draft',
            ]);
        }

        // update info early capture
        $order->full_name        = $request->full_name ?? $order->full_name;
